function calculateLoan() {
    ...
    if (loanAmountInput.value.trim() === '' || interestRateInput.value.trim() === '' || loanTermInput.value.trim() === '') {
        ...
        alertDiv.classList.add('alert-primary'); // Agregar clase para alerta de error
...
function showResultAlert(monthlyPayment, totalPayment, totalInterest, totalMonths) {
    ...
    const alertMessagePayment = `El pago mensual será de: $${monthlyPayment.toFixed(2)} durante ${totalMonths} meses.`;
    const alertMessageInterest = `El total de intereses pagados será de: $${totalInterest.toFixed(2)}.`;
    
    const resultDiv = document.querySelector('#result');
    resultDiv.parentElement.style.display = 'block';
    
    resultDiv.innerHTML = `${alertMessagePayment}<br>${alertMessageInterest}`;
